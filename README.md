...
|-------------------------------------------------------------------------------------------------------------|-----------------------------------------|---------------------------------|-----------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------|
| [changelog.yml](https://github.com/alejandrohdezma/.github/blob/master/workflows/changelog.yml)             | `.github/workflows/changelog.yml`       | Releases and pushes to master   | Fully automated changelog generation. Generates a `CHANGELOG.md` file based on tags, issues and merged pull requests (and separates them into designated categories based on their labels). The exact configuration can be found [here](https://github.com/alejandrohdezma/.github/blob/master/workflows/changelog.yml#L36). An example of a generated changelog file can be found [here](https://github.com/alejandrohdezma/sbt-fix/blob/master/CHANGELOG.md). |
...
| [docs.yml](https://github.com/alejandrohdezma/.github/blob/master/workflows/docs.yml)                       | `.github/workflows/docs.yml`            | Releases and pushes to master   | Runs `sbt ci-docs` on the project and creates a new PR with the changes. This task should be added to the project as a command alias containing the necessary steps to update documentation (re-generate docs files, publish micro-sites, update headers...). And example of this alias can be found [here](https://github.com/alejandrohdezma/sbt-github/blob/master/build.sbt#L7).                                                                            |
...
| [release.yml](https://github.com/alejandrohdezma/.github/blob/master/workflows/release.yml)                 | `.github/workflows/release.yml`         | Releases and pushes to master   | Creates a release of the project by running `sbt ci-release`. This task can be obtained by adding the [`sbt-ci-release` plugin](https://github.com/olafurpg/sbt-ci-release) to the project. This plugin will publish our library to Sonatype and Maven Central using git tags information.                                                                                                                                                                      |
...
After each release a PR will be created in the remote repository with the changes. If [Mergify](https://mergify.io/) is correctly enabled in the repository, this PR will be automatically merged after CI passes.
